...
def cargar_datos(directorio):
    ...
    accesos = users_ref.get()
    # Limpiar el Listbox
    listbox.delete(0, tk.END)
    #listbox.insert(tk.END, f"CODIGO USADO      INQUILINO      FECHA")

    # Insertar los datos en el Listbox
    if accesos:
        for inquilino_key, acceso_data in accesos.items():
            urlFoto= acceso_data.get('Fotografia','')
            codigo_usado = acceso_data.get('Codigo_usado', '')
            nombre_Inquilino = acceso_data.get('nombre_Inquilino', '')
            id_Inquilino = acceso_data.get('id_Inquilino', '')
            #codigo_acceso = acceso_data.get('Id', '')
            fecha = urlFoto[0:-4]
            listbox.insert(tk.END, f"FECHA:{fecha} CÓDIGO: {codigo_usado} INQUILINO: {nombre_Inquilino}")
            ...
            for blob in blobs:
                ...
                urlImg = blob.name[len(directorio):]
                #print("\nURL IMG blob:" +urlImg)
                #print("URL foto:" +urlFoto)
                if urlImg==urlFoto:
                    imgAccesos.append((urlImg, imagen))
                    break

    else:
        listbox.insert(tk.END, "No hay accesos por código registrados")


# Almacena las imágenes de los intrusos
def bajar_imagenes(directorio):
    # Accede a bucket de Firebase y lista archivos que contienen un prefijo, 'directorio'
    bucket = storage.bucket()
    blobs = bucket.list_blobs(prefix=directorio)
    imagenes = []
...
def seleccion(event):
    ...
    if index:
        image = imgAccesos[index[0]][1].resize((400, 300), Image.BICUBIC)
...
root = tk.Tk()
root.title("Accesos por código.")

# Encabezado de sección
header_label = tk.Label(root, text="Accesos por código", font=('Arial', 24), bg='#7DD6FC', fg='white',bd=2, highlightbackground="black")
...
#imagenes = bajar_imagenes('Intrusos/')
cargar_datos('Intrusos/')
